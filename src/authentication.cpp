...
bool authentication::init_auth(const application &app, std::unordered_map<std::string, std::optional<std::reference_wrapper<const entity>>> entity_ref_map, std::vector<prepared_statement_metadata> & stats)
{
    auto & auth = app.auth;
    ...
    if(not auth.provider.empty()){
        ...
        try {
            ...
            stats.push_back(db::init_stmt_login(db::get_database(app), provider, global_statement_index++, auth));
            stats.push_back(db::init_stmt_logout(db::get_database(app), provider, global_statement_index++, auth));
            return true;
        } catch(std::out_of_range & e){
            throw std::runtime_error(fmt::format("no such auth provider \"{}\"", auth.provider, e.what()));
        }
    } else {
