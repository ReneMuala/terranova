...
inline const std::string& get_c_type(const std::string& name) {
    ...
    for (auto& type : types) error_msg += fmt::format("{}({}), ", type.first, type.second);
    error_msg.pop_back();
    error_msg.pop_back();
    throw std::runtime_error(error_msg);
}
...
const field& get_field(
    ...
    for (auto& field : entity.get().schema.fields)
        if (misc::tolower(field.name) == field_name) return field;
    if (auto& pk = entity.get().schema.pk)
        if (misc::tolower(pk->name) == field_name) return (field&)*pk;
...
inline std::string get_field_declaration(const std::string& name, const std::string& type,
    ...
    if (not primary_key and unique) extras.append(" UNIQUE");
    if (required) extras.append(" NOT NULL");
...
bool init_entity(
    ...
    for (auto& rel : entity.schema.has_one) handle_relationship(rel);
    for (auto& rel : entity.schema.has_many) handle_relationship(rel);
    for (auto& rel : entity.schema.belongs_to) handle_relationship(rel);
    if (fields) stmt.pop_back();
    if (not fks.empty()) {
        fks = "," + fks;
        fks.pop_back();
    }
...
prepared_statement_metadata init_stmt_insert(
    ...
    auto handle_relationship = [&](const auto& rel) {
        ...
        if constexpr (std::is_same_v<decltype(rel), const belongs_to&>) {
            ...
            params.emplace_back(field_name, get_c_type(target_field.type),std::string(), std::string(),
...
prepared_statement_metadata init_stmt_delete(const SQLite::Database& database, const entity& entity,
    ...
    if (entity.schema.pk) {
        pk_name = misc::throw_if_invalid_identifier(misc::tolower(entity.schema.pk->name));
        params.emplace_back(pk_name, get_c_type(entity.schema.pk->type),std::string(), std::string(), "required");
    }
    std::string stmt =
...
prepared_statement_metadata init_stmt_custom_sql(
    const SQLite::Database& database, const entity& entity, const std::string& name,
    std::string stmt, const std::vector<param>& params, const std::string& http_method,
...
prepared_statement_metadata init_stmt_custom_composed(
    const SQLite::Database& database, const entity& entity, const std::string& name,
    const std::vector<struct data>& data, const std::vector<param>& params,
    const std::string& http_method, const std::string& comments, const unsigned long long index,
    const prepared_statement_metadata::data_provider_t data_provider_type =
        prepared_statement_metadata::url_params) {
    std::vector<param> stat_params;
...
inline prepared_statement_metadata init_stmt_custom(
    const SQLite::Database& database, const entity& entity, const std::string& name,
    const std::string stmt, const std::vector<param>& params, const std::vector<data>& data,
    const std::string& http_method, const std::string& comments, const unsigned long long index,
    const prepared_statement_metadata::data_provider_t data_provider_type =
        prepared_statement_metadata::url_params) {
    if (not stmt.empty())
...
std::tuple<std::vector<prepared_statement_metadata>, bool> init_statements(
