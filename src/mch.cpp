...
namespace mch
{
    ...
    bool lookup(const std::string& it, std::string const& str, const size_t offset)
    {
        const auto result = std::string_view(str.data() + offset, str.size() - offset).starts_with(it);
        return result;
    }

    bool expect(const std::string& it, std::string const& str, size_t& offset)
    {
        const auto result = std::string_view(str.data() + offset, str.size() - offset).starts_with(it);
        if (result)
        {
    ...
    inline size_t for_each_dotted(std::string const& open_pattern, std::string const& close_pattern,
                              std::string const& str, size_t& offset, std::string id,
                              const std::function<void(const std::string&, bool last)>& callback)
    {
        bool is_last = not expect(".", str, offset);
        callback(id, is_last);
        size_t depth = 0;
        while (not is_last)
        {
    ...
    inline size_t for_each_dotted_reversed(std::string const& open_pattern, std::string const& close_pattern,
                                  std::string const& str, size_t& offset, std::string id,
                                  const std::function<void(const std::string&, bool last)>& callback)
    {
        std::stack<std::tuple<std::string, bool>> buf;
        bool is_last = not expect(".", str, offset);
        buf.push({id, is_last});
        size_t depth = 0;
        while (not is_last)
        ...
        {
            auto & top = buf.top();
            callback(std::get<0>(top), std::get<1>(top));
            buf.pop();
        }
        return depth;
    }
            ...
            if (expect(open_pattern, str, offset))
            {
                ...
                if (expect("=", str, offset))
                {
                    ...
                    for_each_dotted(
                        ...
                        close_pattern,
                        str,
                        offset, extract_id_or_dot(str, offset, open_pattern, close_pattern),
                        [&nodes,&context_stack](
                        const std::string& id, bool last)
                        {
                            open_section(nodes, context_stack, id, last ? mch::open_list:mch::fetch);
                        });
                    ...
                    for_each_dotted_reversed(
                        ...
                        str,
                        offset, extract_id_or_dot(str, offset, open_pattern, close_pattern),
                        [&nodes,&context_stack,&offset](
                        const std::string& id, bool last)
                        {
                            close_section(nodes, context_stack, id, offset);
                        });
                else
                {
                    ...
                    const auto depth = for_each_dotted(open_pattern, close_pattern, str, offset, id,
                                                       [&nodes](const std::string& id, bool last)
                                                       {
                                                           nodes.push_back(node(last ? mch::resolve : mch::fetch, id));
                                                       });
