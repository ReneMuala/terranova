...
template <typename T> bool has_field(const std::string &name) {
    try {
        T type;
...
template <typename T> inline std::string get_canonical_name() {
    using namespace ylt::reflection;
    std::string cann_name =
        misc::snake_to_kebab(fmt::format("{}(", ylt::reflection::get_struct_name<T>()));
    T type;
...
template <typename T> void load(kdl::Node &node, T &type);

template <typename T> void get_loader() {
...
template <typename T> void load(kdl::Node &node, T &type) {
        ...
        for_each(type, [&](auto &field, auto name, auto index) {
            if (prop_name == misc::snake_to_kebab(misc::remove_trailing_underline(name))) {
                ...
                try {
                    ...
                    std::string field_type = {get_struct_name<decltype(field)>().data(),
                                              get_struct_name<decltype(field)>().length() - 1};
                    throw std::runtime_error(
                        fmt::format("property \"{}\" should be \"{}\" (not \"{}\")",
                                    misc::snake_to_kebab(prop_name), field_type,
    ...
    for (auto &child : node.children()) {
        ...
        for_each(type, [&](auto &it, auto name, auto index) {
            ...
            } else if constexpr (std::is_same_v<decltype(it), std::vector<struct role> &>) {
                if (child_name == "role") {
        ...
        if (not success)
            throw std::runtime_error(fmt::format("child \"{}\" is not supported in \"{}\"",
                                                 child_name, get_canonical_name<T>()));
    }
}
...
void load(kdl::Document &document, std::vector<application> &apps) {
    for (auto &node : document.nodes()) {
        if (node.name() == u8"application") {
            ...
            throw std::runtime_error(
                fmt::format("root node type \"{}\" is not supported", (char *)node.name().c_str()));
        }
    }
}
...
void collect_float(const char *name, void *output_buffer, void *input_buffer) {
    if (name && output_buffer && input_buffer)
        *static_cast<float *>(output_buffer) =
            yyjson_get_real(yyjson_obj_get(static_cast<yyjson_val *>(input_buffer), name));
}
...
void collect_const_char(const char *name, void **output_buffer, void *input_buffer) {
    if (name && output_buffer && input_buffer)
        *(char **)output_buffer = fake_strdup(
            yyjson_get_str(yyjson_obj_get(static_cast<yyjson_val *>(input_buffer), name)));
}
...
void prepared_statement_append_results_json(void **result, void **root_field, const char *section,
                                            void *prepared_statement, void *context) {
    SQLite::Statement *stat = (SQLite::Statement *)prepared_statement;
        ...
        while (stat->executeStep()) {
            ...
            for (int i = 0; i < col_count; ++i) {
                ...
                if (type == SQLite::INTEGER)
                    yyjson_mut_obj_add_int(output_doc, data_item, column.getName(),
                                           stat->getColumn(i).getInt());
                else if (type == SQLite::FLOAT)
                    yyjson_mut_obj_add_double(output_doc, data_item, column.getName(),
                                              stat->getColumn(i).getDouble());
                else if (type == SQLite::TEXT or type == SQLite::BLOB)
                    yyjson_mut_obj_add_strcpy(output_doc, data_item, column.getName(),
                                              stat->getColumn(i).getText());
...
bool get_request_body(const char *body, int body_len, request_body_handler_t callback,
...
void collect_session_int(int *field, const char *session_query, int default_, void *context) {
    ...
    // fmt::println("session <int> {} : '{}'", session_query,
    // std::string(session_query+SESSION_PREFIX_SIZE));
}

void collect_session_float(float *field, const char *session_query, float default_, void *context) {
    bool success = false;
    drogon::HttpRequest *ctx = (drogon::HttpRequest *)context;
    if (ctx and ctx->session()) {
...
void collect_session_const_char(const char **field, const char *session_query, char const *default_,
                                void *context) {
    bool success = false;
    drogon::HttpRequest *ctx = (drogon::HttpRequest *)context;
    if (ctx and ctx->session()) {
    ...
    if (not success) {
        *field = fake_strdup(default_);
    }
}

void collect_session_bool(bool *field, const char *session_query, bool default_, void *context) {
    bool success = false;
    drogon::HttpRequest *ctx = (drogon::HttpRequest *)context;
    if (ctx and ctx->session()) {
        ...
        for (const auto &profile : app.profile) {
            if (profile.name == select_profile or (select_profile.empty() and profile.default_)) {
                for (const auto &listen : profile.listen) {
                    if (not registered)
                        registered = true;
                    LOG(INFO) << fmt::format("profile:{} adding listener {}:{}", profile.name,
                                             listen.address, listen.port);
                    drogon::app().addListener(listen.address, listen.port, not listen.cert.empty(),
                                              listen.cert, listen.key);
                }
            }
        }
    }
...
typedef char *(*handler_t)(void *prepared_statement, const char *route, void *context,
                           void *req_context, const char *body, int body_len, size_t *output_size);
...
void log_message(const char *message) { LOG(INFO) << message; }

void log_address(unsigned long long message) { LOG(INFO) << "Address: " << message; }
...
void find_stmt_or_throw(const std::vector<generated_implementation> &queries,
                        const std::string &entity_name, const std::string &query_name,
                        const std::function<void(const generated_implementation &)> callback) {
    bool entity_found = 0;
    ...
    throw std::runtime_error(fmt::format("compiled query \"{}\" not found in entity \"{}\"{}",
                                         query_name, entity_name,
                                         entity_found ? "" : " (inexistent entity)"));
};
std::vector<processed_template> init_views(const std::vector<application> &apps,
                                           const std::vector<generated_implementation> &queries) {
        ...
        for (const auto &entity : app.entity) {
            for (const auto &template_ : entity.views.template_) {
                ...
                if (template_.inline_.length() and template_.file.length())
                    throw std::runtime_error(
                        fmt::format(R"(template for "{}" cannot have both html and file fields)",
                                    template_.name));
                if (template_.inline_.empty() and template_.file.empty())
                    throw std::runtime_error(
                        fmt::format(R"(template for "{}" must have an html or file specified)",
                                    template_.name));
...
void server_mode(const std::string filename, const std::string profile, bool write_c_file) {
 ...
 ┴ └─┘┴└─┴└─┴ ┴┘└┘└─┘ └┘ ┴ ┴)");
    ...
    std::u8string content((std::istreambuf_iterator<char>(file)), std::istreambuf_iterator<char>());

    file.close();
    // std::cout << std::string(reinterpret_cast<const char*>(content.data()),content.size()) <<
    // std::endl;
    ...
    service_layer.push("prepared_statement_get_results_json",
                       (void *)prepared_statement_get_results_json);
    ...
    service_layer.push("raise_unexpected_url_param_error",
    ...
    service_layer.push("prepared_statement_append_results_json",
                       (void *)prepared_statement_append_results_json);
    service_layer.push("prepared_statement_finish_results_json",
                       (void *)prepared_statement_finish_results_json);
            ...
            drogon::app().registerHandler(
                ...
                [handler, prepared_stat, &roles_list,
                 &service_handler_getter](const drogon::HttpRequestPtr &req, Callback &&callback) {
                    ...
                    size_t _r0_size = 0;
                    if (auto _r0 =
                            handler(prepared_stat, ("/?" + req->getQuery()).c_str(),
                                    (void *)resp.get(), (void *)req.get(), req->getBody().data(),
                                    req->getBody().size(), &_r0_size)) {
    ...
    for (vws::processed_template &impl : processed_templates) {
        LOG(INFO) << fmt::format("{} {} -> {}(...)", impl.method, impl.route,
                                 impl.handler.empty() ? "opaque_handler" : impl.handler);
        service_layer.push("get_request_body", (void *)get_request_body);
        handler_t handler =
    ...
    if (apps.size()) {
        drogon::app().enableSession(apps[0].auth.timeout, drogon::Cookie::SameSite::kNull,
                                    "Authorization");
    }
    apps.clear();
    // hks::test();
